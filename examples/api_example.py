...
def natural_sort_key(path):
    ...
    """
    return [int(text) if text.isdigit() else text.lower() for text in re.split('([0-9]+)', path.name)]
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        dest='columns',
        help='Directory of images for a column. Specify this for each column.\nExample: --column ./col1 --column ./col2'
    )
    parser.add_argument('--api-key', required=True, help='Your personal API key for authentication.')
    parser.add_argument('--tags', nargs='*', default=[], help='A space-separated list of tags for the comparison.')
    ...
    response = requests.post(f"{base_url}/comparisons", json=comparison_data, headers=headers, timeout=10)
    ...
    for row_idx in range(num_rows):
        for col_idx in range(num_columns):
            ...
            with open(path, "rb") as f:
                ...
                response = requests.post(upload_url, files=file_data, data=form_data, headers=headers, timeout=30)

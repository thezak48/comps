...
class ComparisonBase(BaseModel):
    ...
    show_name: Optional[str] = Field(
        None, description="Display name shown in the UI", examples=["Example"]
    )
    tags: List[str] = Field(
        default_factory=list,
        description="List of tags for categorizing the comparison",
    ...
    total_rows: int = Field(
        1, description="Number of rows in the comparison grid (max 20)", examples=[1]
    )
    total_columns: int = Field(
        2, description="Number of columns in the comparison grid", examples=[2]
    )
    ...
    expiration_days: int = Field(
        7, description="Number of days until the comparison expires", examples=[7]
    )
...
class ImagePosition(BaseModel):
    filename: str = Field(
        ..., description="Original filename of the image", examples=["image1.jpg"]
    )
    row: int = Field(..., description="Row position in the comparison grid (0-based)", examples=[0])
    column: int = Field(
        ...,
...
class ImageMetadata(BaseModel):
    ...
    image_size: Optional[str] = Field(
        None, description="Size of the image in bytes", examples=["1024 bytes"]
    )
    custom_name: Optional[str] = Field(
        None, description="Custom name for the image", examples=["Left Image"]
    )
...
class ImageDetail(BaseModel):
    filename: str = Field(
        ...,
        description="System filename of the image",
        examples=["55c501a7-86a7-44b7-9aaa-333c96d38f11.jpg"],
    )
    original_filename: Optional[str] = Field(
        None, description="Original filename of the image", examples=["image1.jpg"]
    )
    custom_name: Optional[str] = Field(
        None, description="Custom name for the image", examples=["Left Image"]
    )
    image_size: Optional[str] = Field(
        None, description="Size of the image", examples=["1024 bytes"]
    )
...
class CustomNameUpdate(BaseModel):
    custom_name: str = Field(
        ...,
        description="New custom name for the image",
        examples=["Updated Image Name"],
    )

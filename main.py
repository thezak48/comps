                    ...
                    if expired_ids:
                        ...
                        for comparison_id in expired_ids:
                            try:
                                ...
                                logger.error("Error deleting comparison %s: %s", comparison_id, str(e))
                    else:
                        logger.info("No expired comparisons found")
                except sqlite3.OperationalError as e:
                    if "no such column" in str(e):
                        logger.warning("Skipping cleanup: %s. Migrations may not be complete.", str(e))
                    else:
...
async def api_create_comparison(
    ...
    logger.info("Create comparison request from user: %s", user['username'] if user else 'anonymous')
...
async def api_upload_image(
    ...
    """
    logger.info("Uploading image %s to comparison %s at (%s, %s)", original_filename, comparison_id, row, column)
...
async def upload_files(
    ...
    if request:
        user = await auth.get_optional_user(request)
        logger.info("Upload request from user: %s", user['username'] if user else 'anonymous')
